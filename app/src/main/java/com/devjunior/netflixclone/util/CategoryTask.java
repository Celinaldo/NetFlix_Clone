package com.devjunior.netflixclone.util;
...
import com.devjunior.netflixclone.model.Category;
...
public class CategoryTask extends AsyncTask<String, Void, List<Category>> {
    ...
    private List<Category> getCategories(JSONObject json) throws JSONException {
        ...
        for (int i = 0; i < categoryArray.length(); i++) {
            ...
            JSONArray moviArray = category.getJSONArray("movie");
            for (int j = 0; j < moviArray.length(); j++) {
                JSONObject movie = moviArray.getJSONObject(j);
